...
async def get_embed_colour(guild_id):
    try:
        ...
        async with aiosqlite.connect(db_path) as conn:
            async with conn.execute('SELECT value FROM customisation WHERE type = ? AND guild_id = ?', ("embed_color", guild_id)) as cursor:
...
async def log_command_usage(bot, interaction):
    try:
        ...
        # If log_channel is found, send the log embed
        if log_channel:
            embed = discord.Embed(
        ...
        # Log the command name and interaction data if an error occurs
        command_name = interaction.command.name if interaction.command else "Unknown Command"
        logger.error(f"Unexpected error logging command usage for command '{command_name}': {e}")
...
# ---------------------------------------------------------------------------------------------------------------------
# Custom Predicate for App Commands
# ---------------------------------------------------------------------------------------------------------------------
def has_custom_permissions():
    async def predicate(interaction: discord.Interaction) -> bool:
        # Log the check for debugging
        logging.debug(f"Checking permissions for user {interaction.user.id} in guild {interaction.guild.id}")
        # Perform the permission check
        allowed = await check_permissions(interaction)
        if not allowed:
            # Raise CheckFailure if the user lacks permissions
